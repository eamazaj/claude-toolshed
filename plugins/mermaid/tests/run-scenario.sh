#!/usr/bin/env bash
# run-scenario.sh — test mermaid skill scenarios
#
# Usage:
...
#
# Examples:
#   ./run-scenario.sh red   scenarios/baseline-flowchart.txt
#   ./run-scenario.sh green scenarios/skill-flowchart.txt mermaid-diagram
#   ./run-scenario.sh green scenarios/skill-state.txt    mermaid-diagram

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)" # plugins/mermaid/
...
if [[ "$PHASE" == "red" ]]; then
  ...
  # This is exactly what happens when the skill is active (content injected into system prompt)
  if [[ -z "$SKILL" ]]; then
    echo "Error: green phase requires a skill name (e.g. mermaid-diagram)"
    exit 1
  fi
...
echo "  RED:   Did it skip validation? Wrong save location? Ignore reference guides?"
echo "  GREEN: Did it find PLUGIN_DIR? Read reference guide? Use node scripts? Validate? Correct filename?"
